...
状态：第一阶段 Key、备注搜索已接入独立后端 search 模块，合并表格、分页和历史查询可使用真实接口；标签筛选和统一权限接入待开发
...
- 第一阶段只搜索 Key、备注，标签筛选为下一阶段，不检索加密值，不建立明文值索引
- 建议关键词匹配 Key 或备注；标签使用独立多选，展示 name(code) 和描述
...
- 全局标签候选接口尚需设计，现有 tag/list 要求 tenantId
...
- 范围顶部按 `租户/组织/项目` 显示连续路径，空间不足时自然换行，下方保留文件夹名称路径与 code
...
- 跨范围：查询所选范围下全部环境内的秘钥
- 右侧搜索沿用秘钥管理页的浅色圆角输入框，桌面宽度 176px、最小高度 34px，放大镜位于框内左侧，支持点击图标、Enter 提交和清空，保留 aria-label
- 导航选中态与检索页的蓝色共用 --v-brand-primary，值为 #2663eb
- 范围、秘钥 Key、标签列固定宽度依次为 240px、160px、110px，剩余空间优先留给环境值；长路径与备注可换行，窄屏在表格内部横向滚动
...
| 标签过滤         | 尚未实现筛选；已按 groupId 批量加载标签展示，后续可复用 EXISTS 逻辑避免多标签 JOIN 放大结果                                        |
...
第一阶段接口为 `POST /api/v1/secret/search`，入参如下，不接收 tagIdList 或其他未支持字段
...
- 多范围取并集，再与关键词和环境取交集；本阶段返回组的全部有效标签，标签筛选是后续扩展
...
```mermaid
flowchart TD
    A[前端提交范围、环境、关键词及页码] --> B[Handler 绑定参数并归一化分页]
    B --> C[Service 获取当前用户并校验范围组合]
    C --> D[生成统一筛选条件 并预留权限过滤]
    D --> E[查询去重 groupId 总数与当前页 groupId]
...
4. 已实现：前端使用独立 `src/api/secret-search.ts`，在 `SecretSearchView.vue` 接入真实分页、局部 loading、失败重试与请求取消；旧响应不能覆盖新的范围条件
5. 已实现：检索页历史使用已有历史与批次接口，支持分页追加和按需批次加载；与管理页抽取共用展示组件留待后续，第一步不改原管理页
6. 接入跨租户标签候选与筛选，完成权限、分页和性能验收，再同步接口文档
...
真实 Key、备注搜索已实现，统一权限逻辑和标签筛选尚未开发；Java SDK 当前没有调用此新接口，本阶段不调整 SDK
...
- 模拟数据支持范围与环境筛选、关键词搜索、值显隐和复制，以及历史列表、版本详情、批次详情的弹框切换
- 正式页面历史使用 secret/history 分页，批次详情按需调用 secret/history/batch，Mock 入口保持模拟数据隔离
- 标签筛选和统一权限接入待开发，后端分页及真实历史数据已接入

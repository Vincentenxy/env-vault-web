...
- 标签在“组织管理 → 编辑租户 → 标签管理”中定义
- 新增标签直接使用多行表格，支持增删行和统一提交；已有标签仍使用原编辑弹框
- 未提交的标签草稿写入 localStorage，按当前 userId 和 tenantId 隔离，关闭弹框、刷新或重新登录后再次进入该租户时恢复；保存字段仅包含标签 Code、名称、备注和值检索开关
- 批量创建复用现有 tag/create 接口逐行提交，成功行立即从草稿移除，失败行展示错误并保留，重试只提交剩余行；提交前校验必填、格式、长度与草稿内 Code 重复
- 返回列表和关闭弹框不清除草稿，移除草稿行才会删除对应本地内容；草稿仅在当前浏览器、当前站点可恢复，不跨设备同步
- 密钥列表的标签图标打开独立编辑弹框，只查询标签信息，不读取密钥值；可搜索当前租户标签并分页加载
- 标签按 groupId 绑定，同一个 Key 的所有环境共享；保存空选择解绑全部，不修改值版本
- 标签显示在密钥名称下，工具栏可按当前目录已使用的标签筛选，多选匹配任意一个，筛选由后端执行
- 删除标签会同步移除密钥关联；删除确认展示影响说明，密钥内容与历史不受影响
- 后端新增 secret_tag_relation 表，上线前按后端 design/database.md 执行 DDL；接口详见 design/secret-tag-openapi.yaml
- 目前“允许值检索”仅保存策略，不提供密钥值搜索；资源权限待统一权限中心接入

### 页面导航记忆

- 租户、组织和项目的“用户管理 → 添加成员”在当前管理弹框内切换二级页面，左上角返回成员列表，不再叠加弹框
- 添加成员保留搜索、多选和当前结果全选；同一次打开管理弹框期间返回再进入保留选择，关闭或切换资源后清空
- 候选成员加载失败可重试，提交成功后自动回到列表并刷新；提交期间禁止返回、关闭、切换 Tab 和重复添加
